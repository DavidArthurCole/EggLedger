...
// CheckForUpdates polls the GitHub releases API, compares against the running
// version, and returns the latest version and release notes when an update is
// available.
func CheckForUpdates() (newVersion string, newReleaseNotes string, err error) {
	...
	runningVersion, err := version.NewVersion(appVersion)
	if err != nil {
		err = errors.Wrapf(err, "failed to parse running version %s", appVersion)
		return "", "", wrap(err)
	}

	lastUpdateCheckAt, knownLatestTag, knownLatestReleaseNotes := _host.UpdateCheckSnapshot()
	...
	if runningVersion.GreaterThan(latestVersion) {
		log.Infof("running version %s is newer than latest stable %s, checking pre-releases", appVersion, latestTag)
	...
	_host.SetUpdateCheck(latestTag, latestReleaseNotes)

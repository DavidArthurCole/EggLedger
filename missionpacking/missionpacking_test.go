...
// TestMain loads the embedded eiafx config so the nominal-capacity table can
// self-initialize. In the running app this happens via eiafx.LoadConfig in
// main's init; tests must trigger it explicitly.
func TestMain(m *testing.M) {
	if err := eiafx.LoadConfig(os.TempDir()); err != nil {
		panic(err)
	}
	os.Exit(m.Run())
}
...
// The nominal-capacity table self-initializes on first use.
...
func TestIsDubCap_Normal(t *testing.T) {
	...
	level := uint32(0)
	nominal := nominalShipCapacities[ship][dur][level]

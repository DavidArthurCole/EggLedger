...
// decoded CompleteMissionResponse. Self-initializes the nominal-capacity table.
func ComputeMissionFilterCols(startTimestamp float64, resp *ei.CompleteMissionResponse) (db.MissionFilterCols, bool) {
	...
	if caps, ok := nominalShipCapacities[ship][durationType]; ok && int(level) < len(caps) {
...
// row (no payload decompression needed). Self-initializes the nominal-capacity table.
func MissionMetaToDBMission(meta db.MissionMeta) DatabaseMission {
	...
	var nominalCap float32
	if caps, ok := nominalShipCapacities[ship][durationType]; ok && int(meta.Level) < len(caps) {
...
func GetMissionInformation(ctx context.Context, playerId string, missionId string) DatabaseMission {
	...
	if completeMission == nil {
		// Cache miss for this (player, mission): nothing to compile.
		return DatabaseMission{}
	}
...
func CompileMissionInformation(completeMissionResponse *ei.CompleteMissionResponse) DatabaseMission {
	...
	var nominalCap float32
	if caps, ok := ShipCapacities(info.GetShip(), info.GetDurationType()); ok && int(info.GetLevel()) < len(caps) {
		nominalCap = caps[int(info.GetLevel())]
	}
	mType := resolveMissionType(-1, completeMissionResponse)
	...
	missionInst := DatabaseMission{
		...
		MissionType:       mType,
		MissionTypeString: ei.MissionInfo_MissionType(mType).Display(),
...
func isDubCap(mission *ei.CompleteMissionResponse) bool {
	ensureCaps()
	caps, ok := ShipCapacities(mission.Info.GetShip(), mission.Info.GetDurationType())
	level := int(mission.Info.GetLevel())
	if !ok || level >= len(caps) {
		// Unknown ship/duration/level: cannot determine nominal capacity, so do
		// not flag it as double-capacity (avoids a false positive against a 0
		// nominal, and avoids the previous out-of-range panic).
		return false
	}
	nominalCapcity := caps[level]
	//1.7 to account for rounding errors (1.5x is the max with ER)
	return float32(mission.Info.GetCapacity()) >= (nominalCapcity * 1.7)
}

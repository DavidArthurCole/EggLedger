...
// ExecuteComparison runs a Menno community-data comparison for the given report
// and returns the serialized ReportResult, or an empty string on any failure.
func ExecuteComparison(reportId, rawRowLabelsJSON, rawColLabelsJSON string) string {
	...
	filePath := filepath.Join(_host.InternalDir(), "menno-data.json")

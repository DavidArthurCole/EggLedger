...
// Init must be called after the DB is open. If internalDir/storage.json exists
// it is loaded, migrated to DB in one batch, then renamed to
// storage.json.migrated so it is never processed again.
func (s *AppStorage) Init(internalDir string) {
	storageFile := filepath.Join(internalDir, "storage.json")
	if _, err := os.Stat(storageFile); err == nil {
		if s.loadFromJSONFile(storageFile) {
			s.persistAllToDB()

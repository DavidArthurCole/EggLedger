...
func TestListExportGroups_MissingDir(t *testing.T) {
	groups, err := ListGroups(t.TempDir())
	if err != nil {
...
func TestPruneExportsForPlayer_DeletesOldest(t *testing.T) {
	...
	deleted, freed, err := PruneForPlayer(root, "EI123", 2)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if deleted != 2 {
...
func TestPruneExportsForPlayer_NoOp(t *testing.T) {
	root := t.TempDir()
	deleted, freed, err := PruneForPlayer(root, "EI123", 0)
...
func TestPruneExportsForPlayer_BelowLimit(t *testing.T) {
	...
	deleted, _, err := PruneForPlayer(root, "EI123", 5)

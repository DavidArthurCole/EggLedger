    ...
    <div class="max-h-60 overflow-y-auto">
        <div class="relative flex-grow max-h-6/10" v-for="(_, index) in filterArray" :key="index">
            ...
            <div class="ml-2rem filter-container focus-within:z-10"
                v-for="(orFilter, orIndex) in generateOrFiltersConditionsArr(index)" :key="orIndex">
                <!-- Separator -->
    ...
    export default defineComponent({
        ...
        methods: {
            artifactDisplayText(artifact: { displayName: string; level: number | string }) {
              ...
              const levelOffset = (nameLower.includes('stone') && !nameLower.includes('fragment')) ? 2 : 1
            ...
            changeFilterValue(index: number, orIndex: number | null, level: string, value: string) {
            ...
            isFilterIncomplete(index: number, orIndex: number | null) {
                ...
                return top != null && (op == null || val == null || ((top === 'returnDT' || top === 'launchDT') && val === ''))
            },
            ...
            filterValueIf(top: string | null, op: string | null, vtype?: string): boolean {
                ...
                switch (vtype) {
                    case 'base': return this.isBaseFilter(top)
                    case 'drops': return top === 'drops'
                    case 'target': return top === 'target'
                    case 'date': return top === 'launchDT' || top === 'returnDT'
                    default: return true
                }
            },
            filterLevelIf(index: number, orIndex: number | null, level: string, vtype?: string): boolean {
                const { top, op, val } = this.resolveFilterRefs(index, orIndex)
                switch (level) {
                    case 'top': return true
                    case 'operator': return top != null
                    case 'value': return this.filterValueIf(top, op, vtype)
                    case 'or': return orIndex == null && top != null && op != null && val != null && (top !== 'returnDT' && top !== 'launchDT' || val !== '')
                    case 'clear': return orIndex != null || top != null
